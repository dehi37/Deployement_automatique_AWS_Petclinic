...
variable "blocked_ips" {
  description = "Liste des IPs à bloquer par le WAF (format CIDR)"
  type        = list(string)
  default     = []
}

variable "allowed_countries" {
  description = "Liste des codes pays autorisés pour l'accès via WAF"
  type        = list(string)
...
variable "enable_geo_restriction" {
  description = "Activer la restriction géographique via CloudFront"
  type        = bool
  default     = true
}
...
variable "github_repo" {
  description = "Nom du dépôt GitHub (ex: votre-user/spring-petclinic-infra)"
  type        = string
  default     = "mpididehi/spring-petclinic-infra"  # À modifier
}